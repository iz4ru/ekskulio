        ...
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            <form action="{{ route('reports.preview') }}" method="GET">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Tahun Ajaran <span
                                class="text-red-500">*</span></label>
                        <select name="academic_year_id" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md"
                            required>
                            @foreach ($academicYears as $ay)
                                <option value="{{ $ay->id }}"
                                    {{ request('academic_year_id', $activeAY?->id) == $ay->id ? 'selected' : '' }}>
                                    {{ $ay->display_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Ekstrakurikuler <span
                                class="text-red-500">*</span></label>
                        <select name="extracurricular_id" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md"
                            required>
                            <option value="">-- Pilih Terlebih Dahulu --</option>
                            @foreach ($extracurriculars as $extracurricular)
                                <option value="{{ $extracurricular->id }}"
                                    {{ request('extracurricular_id') == $extracurricular->id ? 'selected' : '' }}>
