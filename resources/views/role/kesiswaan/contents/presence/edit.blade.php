                            ...
                            <tbody>
                                @foreach($presence->details as $detail)
                                    <tr class="hover:bg-gray-100 transition-colors transition-duration-300">
                                        <td class="font-medium text-gray-800 whitespace-nowrap">{{ $loop->iteration }}</td>
                                        <td class="font-medium text-gray-800 whitespace-nowrap">{{ $detail->student->id_number }}</td>
                                        <td class="font-medium text-gray-800 whitespace-nowrap">{{ $detail->student->name }}</td>
                                        <td class="font-medium text-gray-800 whitespace-nowrap">{{ $detail->student->studentClass->name ?? '-' }}</td>
                                        <td class="font-medium text-gray-800 whitespace-nowrap">
                                            <div class="flex items-center justify-center gap-2">
                                                @foreach(['present' => ['Hadir', 'green'], 'sick' => ['Sakit', 'yellow'], 'permission' => ['Izin', 'blue'], 'absent' => ['Alfa', 'red']] as $status => $label)
                                                    <label class="flex items-center px-3 py-2 border-2 rounded-md cursor-pointer transition-all duration-200 hover:shadow-md
                                                        ...
                                                        <input type="radio" name="attendance[{{ $detail->student->id }}]" value="{{ $status }}"
                                                            class="w-4 h-4 text-{{ $label[1] }}-600 bg-gray-100 border-gray-300 focus:ring-{{ $label[1] }}-500 focus:ring-2 mr-2"
                                                            {{ old("attendance.{$detail->student->id}", $detail->status) == $status ? 'checked' : '' }} required>
                                                        <span class="text-xs font-medium text-{{ $label[1] }}-700">{{ $label[0] }}</span>
                                                    </label>
                                                @endforeach
                                            </div>
                                        </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateStats() {
        ...
        const permission = document.querySelectorAll('input[type="radio"][value="permission"]:checked').length;
