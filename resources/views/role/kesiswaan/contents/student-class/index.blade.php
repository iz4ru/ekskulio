...
@section('content')
    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-md w-full mt-14">
        ...
        <div class="flex gap-4 mb-4 justify-between">
            <div>
                <h1 class="text-xl lg:text-2xl font-semibold text-gray-600">Kelola Kelas</h1>
                <p class="text-sm lg:text-base text-gray-400">Tambah, ubah, dan hapus data kelas.</p>
            </div>
        </div>
        ...
        <div class="inline-flex flex-col lg:flex-row gap-2">
            ...
            <a href="{{ route('student-class.import') }}"
                class="inline-flex cursor-pointer items-center px-5 py-2.5 gap-2 text-sm font-medium text-center text-white bg-[#0083E9] rounded-md focus:ring-4 focus:ring-blue-300 hover:bg-[#DEECFF] hover:text-[#0083E9] active:scale-[0.98] transition-all duration-300 ease-out">
                <i class="fa-solid fa-file-import text-sm"></i>
                <span>Impor Kelas</span>
            </a>
        </div>
                ...
                <table id="pagination-table" class="min-w-full text-sm text-left text-gray-600">
                    <thead class="border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-xs uppercase">Nama Kelas</th>
                            <th class="px-4 py-3 text-xs uppercase">Jumlah Siswa</th>
                    ...
                    <tbody>
                        @foreach ($classes as $class)
                            <tr class="hover:bg-gray-100 transition-colors transition-duration-300 border-b border-gray-200">
                                ...
                                <td class="px-4 py-3 font-medium whitespace-nowrap border-gray-200">
                                    <form action="{{ route('student-class.toggle', $class) }}" method="POST"
                                        id="toggle-form-{{ $class->id }}">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="password" id="password-{{ $class->id }}"
                                            value="">
                                ...
                                <td>
                                    <div class="flex gap-3 items-center justify-start">
                                        ...
                                        <form action="{{ route('student-class.destroy', $class->id) }}" method="POST"
                                            id="delete-form-{{ $class->id }}" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="password"
                                                id="delete-password-{{ $class->id }}" value="">
                                            <button type="button"
                ...
                document.addEventListener('change', async function(e) {
                    if (e.target.classList.contains('toggle-checkbox')) {
                        ...
                        const {
                            ...
                            html: `<p>Anda akan <strong>${status}</strong> kelas <strong>${className}</strong>.</p>
                ...
                document.addEventListener('click', async function(e) {
                    ...
                    if (deleteBtn) {
                        ...
                        const {
                            ...
                            title: 'Hapus Kelas',
                            html: `<p>Anda akan menghapus kelas <strong>${className}</strong>.</p>
