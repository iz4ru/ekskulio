...
@section('content')
    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-md w-full mt-14">
        ...
        <div class="w-full space-y-3">
            ...
            @if ($errors->any())
                <div class="alert alert-danger alert-dismissible relative mb-4 w-full text-sm py-2 px-4 bg-red-100 text-red-500 bordered-500 rounded-md opacity-0 transition-opacity duration-150 ease-in-out"
            ...
            <form method="GET" class="mb-4 flex flex-wrap gap-3 items-end">
                    ...
                    <select name="extracurricular_id" id="extracurricular_id"
                        ...
                        @foreach ($extracurriculars as $ekskul)
                            <option value="{{ $ekskul->id }}"
                                {{ request('extracurricular_id') == $ekskul->id ? 'selected' : '' }}>
                                {{ $ekskul->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex-1 min-w-[150px]">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        placeholder="Cari Nama / NIS / Ekskul..."
                        class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md">
                </div>
                ...
                <table id="pagination-table" class="min-w-full text-sm text-left text-gray-600">
                    ...
                    <tbody>
                        @forelse ($memberships as $membership)
                            ...
                            <tr
                                class="hover:bg-gray-100 transition-colors transition-duration-300 border-b border-gray-200{{ $student?->trashed() ? ' bg-red-50' : '' }}">
                                <td class="px-4 py-2 border-gray-200">{{ $loop->iteration }}</td>
                                <td class="px-4 py-2 font-medium whitespace-nowrap border-gray-200">
                                    {{ $student?->id_number ?? '-' }}
                                </td>
                                <td
                                    class="px-4 py-2 font-medium whitespace-nowrap border-gray-200{{ $student?->trashed() ? ' text-red-500 line-through' : '' }}">
                                    {{ $student?->name ?? '-' }}
                                    @if ($student?->trashed())
                                        <span class="ml-1 text-xs text-red-500">(Dihapus)</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2 whitespace-nowrap border-gray-200">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex gap-2 items-center">
                                        @if (!$student?->trashed() && $membership->status_label === 'aktif')
                                            <form action="{{ route('memberships.update.status', $membership) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-8 text-gray-500 border-gray-200">
