...
class ScoreController extends Controller
{
    public function index(Request $request)
    {
                ...
                return $q->where(function ($q) use ($search) {
                    $q->whereHas('membership', function ($m) use ($search) {
                        $m->whereHas('student', function ($s) use ($search) {
                            $s->whereRaw('LOWER(name) LIKE ?', ["%{$search}%"])->orWhereRaw('LOWER(id_number) LIKE ?', ["%{$search}%"]);
        ...
        $view = match ($user->role) {
            'kesiswaan' => 'role.kesiswaan.contents.score.index',
            'pembina' => 'role.pembina.contents.score.index',
            default => abort(403),
        };

        return view($view, compact('scores', 'extracurriculars', 'activeAY', 'selectedAY', 'academicYears', 'studentClasses', 'isAdvisorSingleExtracurricular'));
    }
    ...
    public function input(Request $request)
    {
        ...
        $view = match ($user->role) {
            'kesiswaan' => 'role.kesiswaan.contents.score.input',
            'pembina' => 'role.pembina.contents.score.input',
            default => abort(403),
        };
    ...
    public function getStudents(Request $request)
    {
        ...
        $existingScores = ScoreSummary::where('academic_year_id', $academicYearId)->whereIn('membership_id', $memberships->pluck('id'))->get()->keyBy('membership_id');
    ...
    public function store(Request $request)
    {
        ...
        $scoresInput = collect($request->scores)->filter(fn($s) => !empty($s['score']) || !empty($s['notes']));
        $savedCount = $scoresInput->count();

        // Ambil nama ekskul untuk log — dari membership pertama yang valid
        $firstMembershipId = $scoresInput->keys()->first();
        $extracurricularName = null;
        if ($firstMembershipId) {
            $extracurricularName = ExtracurricularMembership::with('extracurricular')->find($firstMembershipId)?->extracurricular?->name;
        }

        DB::transaction(function () use ($user, $scoresInput, $activeAY, $extracurricularName, $savedCount) {
            foreach ($scoresInput as $membershipId => $scoreData) {
            ...
            Log::create([
                'user_id' => $user->id,
                'activity' => 'Input nilai',
                'detail' => $user->name . ' menyimpan ' . $savedCount . ' nilai' . ($extracurricularName ? ' untuk ' . $extracurricularName : '') . ' tahun ajaran ' . $activeAY->year . ' ' . ucfirst($activeAY->semester),
            ]);
        });
    ...
    public function export(Request $request)
    {
        ...
        if ($type === 'pdf') {
            $pdf = Pdf::loadView('role.kesiswaan.contents.score.pdf', compact('scores', 'academicYear', 'extracurricular', 'studentClass'));
        ...
        $rows[] = ['', '', '', '', '', '', '', '', ''];
        $rows[] = ['No', 'NIS', 'Nama Siswa', 'Kelas', 'Ekstrakurikuler', 'Tahun Ajaran', 'Nilai', 'Predikat', 'Catatan'];

        $no = 1;
        foreach ($scores as $score) {
            $rows[] = [$no++, $score->membership->student->id_number ?? '-', $score->membership->student->name ?? '-', $score->membership->student->studentClass?->name ?? '-', $score->membership->extracurricular->name ?? '-', $score->academicYear->display_name ?? '-', $score->score ?? '-', $score->predicate ?? '-', $score->notes ?? '-'];
        }

        return Excel::download(new ScoreExport($rows), $filename . '.xlsx');
    }
}
